...
parser.add_argument("--logfile", type=str, help="file to log to", default="gp_opt.log")
...
def neg_ei(x, gp, fmin, check_type=True):
    ...
    std_normal = tfp.distributions.Normal(loc=0.0, scale=1.0)
    mu, var = gp.predict_f(x)
    sigma = tf.sqrt(var)
    z = (fmin - mu) / sigma

    ei = (fmin - mu) * std_normal.cdf(z) + sigma * std_normal.prob(z)
    return -ei
...
def robust_multi_restart_optimizer(
    ...
    for i, (x, src) in enumerate(zip(all_points, point_sources)):
        ...
        result_is_good = res.success and (
            "REL_REDUCTION_OF_F_<=_FACTR*EPSMCH" in res.message.decode()
        )
        if result_is_good:
            num_good_results += 1
    ...
    opt_vals_out = [res.fun.sum() for res in sorted_results[:num_pts_to_return]]
