...
class VarAutoEncoderModule(pl.LightningModule):
    ...
    def shared_step(self, batched_data):
        ...
        for b in range(len(counts)):
            ...
            edge_mol_sparse = (edge_targets_sparse[tmp_mask]).reshape(-1,2)
            
            mol_correct = 1
            for i in range(idx, idx + counts[b]):
                for j in range(idx, idx + counts[b]):
                    if i == j:
                        continue
                    elem_total += 1
                    if [i,j] in edge_mol_sparse.tolist():
                        logit = torch.inner(codes[i], codes[j]).unsqueeze(0)
                        edge_recon_loss += torch.nn.functional.binary_cross_entropy_with_logits(logit, torch.Tensor([1]).cuda())
                        pred = (logit >= 0.5).long().item()
                        if pred == 1:
                            elem_acc += 1
                        else:
                            mol_correct = 0
                        ...
                        logit = torch.inner(codes[i], codes[j]).unsqueeze(0)
                        edge_recon_loss += torch.nn.functional.binary_cross_entropy_with_logits(torch.inner(codes[i], codes[j]).unsqueeze(0), torch.Tensor([0]).cuda())
                        pred = (logit < 0.5).long().item()
        ...
        loss += edge_recon_loss/elem_total
        ...
        statistics["acc/edge_elem"] = elem_acc / elem_total
        statistics["acc/edge_mol"] = mol_acc / mol_total
