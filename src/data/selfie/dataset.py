...
class SelfieDataset(torch.utils.data.Dataset):
    def __init__(self, data_dir, split, smiles_transform_type="none", seq_transform_type="none"):
        super(SelfieDataset, self).__init__()
        self.smiles_list = load_smiles_list(data_dir, split)
        self.tokenizer = load_selfie_tokenizer(data_dir)
        self.vocabulary = load_selfie_vocabulary(data_dir)
    ...
    def __getitem__(self, idx):
        ...
        sequence = selfie_sequence_from_smiles(smiles, self.tokenizer, self.vocabulary)
