...
class BackBoneModel(pl.LightningModule):
    def __init__(self, hparams):
        ...
        self.encoder = GnnEncoder(
            num_layer=hparams.encoder_num_layer, emb_dim=hparams.encoder_emb_dim, code_dim=hparams.code_dim
        )
        ...
        self.elem_score_func, self.parallel_score_func = get_scoring_func(hparams.score_func_name, num_workers=32)

        # setup datasets
        (self.train_smiles_list, self.val_smiles_list), (self.train_score_list, self.val_score_list) = load_split_smiles_list(
            hparams.data_dir, score_func_name=hparams.score_func_name
            )
        
        self.train_sequence_dataset = SequenceDataset(self.train_smiles_list, self.tokenizer, self.vocabulary) 
        self.val_sequence_dataset = SequenceDataset(self.val_smiles_list, self.tokenizer, self.vocabulary)
