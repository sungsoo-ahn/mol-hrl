...
class SequenceEncoder(nn.Module):
    def __init__(self, hparams):
        ...
        self.encoder = nn.Embedding(num_vocabs, hparams.encoder_hidden_dim)
        self.lstm = nn.LSTM(
    ...
    def encode_smiles(self, smiles_list, device):
        sequences = [smiles2sequence(smiles, self.tokenizer, self.vocabulary) for smiles in smiles_list]
